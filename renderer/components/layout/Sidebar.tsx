    ...
    <div className="flex flex-col h-full" style={{ background: 'var(--bg-elevated)' }}>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <Section title={i18n.branch.local} expanded={isExpanded('branches')} onToggle={() => toggleSection('branches')} icon={<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>}>
          <BranchSection onShowDialog={onShowDialog} />
        </Section>
        {tags && tags.length > 0 && (
          <Section title="标签" expanded={isExpanded('tags')} onToggle={() => toggleSection('tags')} icon={<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A2 2 0 013 12V7a4 4 0 014-4z" /></svg>}>
            <TagsSection onShowDialog={onShowDialog} />
          </Section>
        )}
...
/* ======================== 分支列表 — 右键触发专业弹窗 ======================== */
function BranchSection({ onShowDialog }: { onShowDialog?: (dialog: string, payload?: string | null) => void }) {
  ...
  return (
    <>
      {localBranches.map((branch) => (
        <BranchItem key={branch.name} branch={branch} onDoubleClick={() => checkoutLocalBranch(branch.name)} onRefresh={refresh} showDialog={showBranchDialog} />
      ))}
...
function BranchItem({ branch, onDoubleClick, onRefresh, showDialog }: {
...
/* ======================== 标签列表 — 右键触发专业弹窗 ======================== */
function TagsSection({ onShowDialog }: { onShowDialog?: (dialog: string, payload?: string | null) => void }) {
  ...
  return (
    <>
      {tags.map((tag) => (
        <TagItem key={tag.name} tag={tag} onRefresh={refresh} showDialog={showTagDialog} />
      ))}
    </>
  );
}

function TagItem({ tag, onRefresh, showDialog }: { tag: { name: string }; onRefresh: () => Promise<void>; showDialog: (dialog: string, payload?: string | null) => void }) {
